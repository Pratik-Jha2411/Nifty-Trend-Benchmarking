...
def fetch_data(ticker="^CRSLDX"):
    ...
    try:
        ...
        if df.empty:
            print("Data empty, trying fallback ^CNX500")
            df = yf.download("^CNX500", start="2010-01-01")
...
def main():
    ...
    y_train_cpu = y_train.cpu().numpy().flatten()
    class_counts = np.bincount(y_train_cpu.astype(int))
    num_samples = len(y_train_cpu)
    
    class_weights = 1. / class_counts
    samples_weights = class_weights[y_train_cpu.astype(int)]
    sampler = WeightedRandomSampler(torch.from_numpy(samples_weights).type(torch.DoubleTensor), num_samples)
    
    print(f"Class Imbalance - Counts: {class_counts}")
    ...
    plt.tight_layout()
    plt.savefig('nifty500_model_comparison.png') # Changed filename
    plt.show()
